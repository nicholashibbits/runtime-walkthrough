/* ================================================================
   ...
   Named exports: analyze, highlightSyntax
   ================================================================ */
...
function groupStatements(lines: string[]) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!current) {
      current = { startLine: i, endLine: i, lines: [lines[i]], fullText: lines[i] };
      parenDepth = 0;
      braceDepth = 0;
    } else {
...
function extractStringLiteral(s: string) {
  const m = s.match(/^(['"`])([\s\S]*)\1$/);
  return m ? m[2] : s;
}

function extractConsoleArg(fullText: string) {
  const m = fullText.match(/console\.\w+\s*\(\s*([\s\S]*?)\s*\)\s*;?\s*$/);
  if (!m) return fullText;
  return extractStringLiteral(m[1].split(',')[0].trim());
}

function extractConsoleMethod(fullText: string) {
  const m = fullText.match(/console\.(\w+)/);
  return m ? m[1] : 'log';
}

function extractTimerDelay(fullText: string) {
  const m = fullText.match(/,\s*(\d+)\s*\)\s*;?\s*$/);
  return m ? m[1] : '0';
}

function extractVarParts(firstLine: string) {
...
function extractFuncName(text: string) {
  const m = text.match(/function\s+(\w+)/);
  return m ? m[1] : null;
}

function extractCallName(text: string) {
...
function classify(stmt: RawStatement): any {
  ...
  if (/setTimeout\s*\(/.test(first) || /^\s*(const|let|var)\s+\w+\s*=\s*setTimeout/.test(first)) {
    ...
    const callbackLines = stmt.lines.length > 1
      ...
      : [];
    let inlineExpr: string | null = null;
  ...
  if (/setInterval\s*\(/.test(first)) {
    const delay = extractTimerDelay(full);
    return { ...stmt, type: 'setInterval', delay, callbackLines: [] as any[], inlineExpr: null as string | null };
  }
...
function analyzeCallbackBody(cbLines: { text: string }[], inlineExpr: string | null) {
  const actions: any[] = [];
...
function generateSteps(classified: any[], _totalLines: number) {
  const steps: any[] = [];
  const microtasks: any[] = [];  // callbacks deferred to microtask queue
  const macrotasks: any[] = [];  // callbacks deferred to task queue

  // ── Step: push global execution context ──
  steps.push({
    line: classified.find((s: any) => s.type !== 'empty' && s.type !== 'comment')?.startLine ?? 0,
  ...
  // ── Step: pop global ──
  const lastSyncLine = [...classified].reverse().find((s: any) =>
  ...
  for (let i = 0; i < microtasks.length; i++) {
    ...
    if (cbActions.some((a: any) => a.type === 'consoleLog')) glowSet.push('boxConsole');
